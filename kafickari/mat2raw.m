function mat2raw(im,path,name)
    ...
    if imDim == 2
     ...
     mhd{6} = 'ElementSpacing = 1 1'; % 1 pixel ma v realu 1 mm
     mhd{7} = 'TransformMatrix = 1 0 0 1';
     mhd{8} = 'Offset = 0 0';
     mhd{9} = 'CenterOfRotation = 0 0';
    elseif imDim == 3
     mhd{3} = ['DimSize = ' num2str(imSize(2)) ' ' num2str(imSize(1)) ' ' num2str(imSize(3))];
    ...
    mhd{12} = 'AnatomicalOrientation = RAI'; %right, anterior, posterior a nebo n2kdy podle DICOM
    mhd{13} = ['ElementDataFile ='  name '.raw']; %name file musi byt na posledni radku
    ...
    im = im2double(im);
    im = uint16(im.*(2^15));
    im = permute(im, [2 1 3]); %prohodi prvni a druhou dimenzi

    fileID = fopen([path c name '.raw'], 'w+');
    fwrite(fileID, im(:), 'uint16'); %zapisujeme matici jako vektor
    fclose(fileID);
